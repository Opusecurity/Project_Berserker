...
if len(df) >= 3:
    ...
    print("[✓] Post-AI model trained on split data.", flush=True)
    ...
    print("[✓] Post-AI model trained on full data.", flush=True)
    print("[~] Classification report:", flush=True)
...
# === CLASSIFICATION METRICS GRAFİĞİ (Post-AI) ===
...
for i, (metric, name, color) in enumerate(zip(metrics, metric_names, colors)):
    bars = plt.bar(x + (i - 1) * width, metric, width=width, label=name, color=color)
    for bar in bars:
        height = bar.get_height()
        plt.text(bar.get_x() + bar.get_width() / 2, height + 0.02, f"{height:.2f}",
                 ha='center', va='bottom', fontsize=10)

plt.title("Post-AI Classification Metrics per Risk Class", fontsize=14, fontweight='bold')
